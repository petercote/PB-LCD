...
void rotate() {
  ...
  } else                   //MENU COUNTER----------------------------------------------------------------------------
  {
    ...
    if (CLKNow != CLKPrevious && CLKNow == 1) {
      ...
      // the encoder is rotating in A direction, so we increase
      if (digitalRead(RotaryDT) != CLKNow) {
        if (menuCounter < 7)  //we do not go above 6
        {
          menuCounter++;
        } else {
          menuCounter = 7;
...
void updateCursorPosition() {
    ...
    switch (menuCounter)  //this checks the value of the counter (0, 1, 2 or 3)
    {
      ...
      case 3:
        ...
        //-------------------------------
      case 4:
        lcd.setCursor(0, 0);  //4th line, 1st block
        lcd.print(">");
        break;
      case 5:
        lcd.setCursor(0, 1);  //4th line, 1st block
        lcd.print(">");
        break;
         //-------------------------------
      case 6:
  ...
  void setup() {
    ...
    pg1LCD();  //print the stationary parts on the screen
    //------------------------------------------------------
  ...
  void loop() {
    if ((refreshLCD == true) && (menuCounter <= 3))  //If we are allowed to update the LCD ...
    {
      pg1LCD();
    ...
    if (refreshLCD == true && (menuCounter >= 4))  //If we are allowed to update the LCD ...
    {
      pg2LCD();  // ... we scroll the LCD ...
      updateLCD();
      Serial.println("pg2 loop");
